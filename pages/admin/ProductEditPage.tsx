...
import type { ProductDtoRequest, ProductFormState, ProductMediaDto } from '../../types/admin';
...
const ProductEditPage: React.FC = () => {
  ...
  const [existingMedia, setExistingMedia] = useState<ProductMediaDto[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [cats, allTags, product] = await Promise.all([
          getCategories(),
          getTags(),
          getProduct(id),
        ]);
        ...
        const categoryName =
          ...
          (typeof product.category === 'string'
            ...
            : typeof product.category === 'object' && product.category !== null
              ...
              : '');
        const catObj = product.category as { id?: string } | null | undefined;
        const idFromCategory = catObj && typeof catObj === 'object' && catObj.id ? catObj.id : undefined;
        const categoryId =
          (product as { categoryId?: string }).categoryId ??
          idFromCategory ??
          cats.find((c) => c.name === categoryName)?.id ??
          '';
        setFormData({
          name: product.name,
          ...
          benefits: (() => {
            const b = product.benefits as string | string[] | undefined;
            if (typeof b === 'string') return b.split(',').map((s) => s.trim()).filter(Boolean);
            return Array.isArray(b) ? b : [];
          })(),
          usage: (product as { howToUse?: string }).howToUse || product.usage || '',
          ingredients: product.ingredients || '',
          inStock: product.inStock,
        });
        setSelectedTagIds(product.tags?.map((t) => t.id) || []);
        setExistingMedia(product.productMedias ?? []);
        setPendingFilenames([]);
      } catch (err) {
      ...
      if (pendingFilenames.length > 0) {
        try {
          await attachProductMediaAll(
            pendingFilenames.map((mediaName) => ({ mediaName, productId: id }))
          );
        } catch (mediaErr) {
          setError(
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const filenames = await uploadImages(Array.from(files));
  ...
  const handleRemoveExistingMedia = async (media: ProductMediaDto) => {
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-serif text-brand-charcoal">Edit Product</h1>
        <p className="text-brand-charcoal/60 mt-2">Update product information</p>
      </div>
            ...
            <div className="mt-4 flex flex-wrap gap-4">
              {existingMedia.map((media) => (
                <div key={media.id} className="relative group">
                  <img
                    src={getUploadImageUrl(media.mediaName)}
                    alt=""
